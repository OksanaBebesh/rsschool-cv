...
I can learn subjects by myself, using books,  youtube channel

I know 
CSS, HTML, php, JavaScript, MYSQL(SQL), Laravel
...
##My education##
I have next kind of **diploma**:
1. secondary special education diploma - Vocational college of Grodno (now College of Engineering Technology and Design) with the assignment of software technician  in 2005**
...
2005 - june 2019 **Vocational college of Grodno (now College of Engineering Technology and Design)** 
    in 2005 In position industrial training master
    in 2008 in position teacher;
    in 2010 - 2013  maternity leave - first child
    at 2013 - 2015  in position teacher;
    2015-2018  maternity leave -> second child;
    
jule 2019 - march 2020 **Velox Imaging**  - Branch Migration. I create procedures, sql queries. My task was to made perfect data placement. 
For example some SQL queries:
```
// This SQL query show information about files, which is not added to table file linksd 
SELECT id_files, study_description, modality, id_referrer
FROM file_tags ft LEFT JOIN link_file lf on ft.id_files = lf.id_files
WHERE lf.id_files is null;

// SQL Query For finding duplicate text reports 
SELECT d.ID_Encounter
FROM qxu.documentdata dd
  JOIN  qxu.documentprocedure dp ON dd.ID_DocumentData=dp.ID_DocumentData
GROUP BY dp.ID_Procedure,dd.TextData
  HAVING COUNT(*)>1;

//SQL Query For finding errors with helth cards, when cards have two patients
...
```
I know English from  school, but not perfect. I like English language and I want to bring to perfection it. Today, I have level: Between A1 and A2.
